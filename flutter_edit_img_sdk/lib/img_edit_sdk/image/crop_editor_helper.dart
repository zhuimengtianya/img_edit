...
/*
Future<Uint8List> cropImageDatWithOpenCV(BuildContext context,
    ExtendedImageEditorState state, List<Offset> points, String path) async {
  ...
  cropPoint[2] = points[EditActionDetails.TOP_RIGHT].dx.toInt(); //rightTop x
  cropPoint[3] = points[EditActionDetails.TOP_RIGHT].dy.toInt(); //rightTop y
  cropPoint[4] =
  ...
  imgEditCrop(state.rawByteData, state.image.width, state.image.height,
      cropPoint, _outBitmap, _newWidth, _newHeight);
  gimage.Image im = gimage.Image.fromBytes(_newWidth, _newHeight, _outBitmap);
  _outBitmap = gimage.encodeJpg(im, quality: 100);
  return _outBitmap;

  print("crop width:$_newWidth,height:$_newHeight");
  print("_outBitmap:$_outBitmap");

  ImageConfiguration config = ImageConfiguration(
    size: Size(_newWidth.toDouble(), _newHeight.toDouble())
  );

  return CommonUtils.loadImageByProvider(MemoryImage(_outBitmap, scale: 1),
  ...
  await file.writeAsBytes(_outBitmap.buffer.asUint8List());
}*/
...
Future<List<int>> cropImageDataWithNativeLibrary(
  ...
  debugPrint('action.needCrop = ${action.needCrop}');
  ...
  if (action.hasRotateAngle) {
    ...
    if (rotateAngle == 90 || rotateAngle == 270) {
      GlobalVariables.rect = Rect.fromLTWH(
          0.0, 0.0, GlobalVariables.rect.height, GlobalVariables.rect.width);
    }
  }
