...
import 'opencv/native_with_opencv.dart';
...
import 'image_cover/extended_image_border_painter.dart';
...
//            autoCancel: autoCancel,
  ...
  ///test iamge bytearray
  Uint8List _imageArray;
...
class _ExtendedImageState extends State<ExtendedImage>
  ...
  @override
  void initState() {
    GlobalVariables.smartCropPoints = null;
  ...
  void updateSmartScropPoint() {
    if (_imageInfo == null) {
      return;
    }
    Future<ByteData> data = _imageInfo.image.toByteData();
    data.then((value) {
      Uint8List sourceBitmap = value.buffer.asUint8List();
      List<int> cropPoint = new List(8);

//      Future<Uint8List> cropRectImage = recognizeHed();
//      cropRectImage.then((value) {
        imgEditSacn(sourceBitmap, _imageInfo.image.width, _imageInfo.image.height,
        cropPoint, true);
        setState(() {
          _imageByteData = sourceBitmap;
          GlobalVariables.smartCropPoints = List(4);
          GlobalVariables.smartCropPoints[EditActionDetails.TOP_LEFT] =
          new Offset(cropPoint[0].toDouble(), cropPoint[1].toDouble());
          GlobalVariables.smartCropPoints[EditActionDetails.TOP_RIGHT] =
          new Offset(cropPoint[2].toDouble(), cropPoint[3].toDouble());
          GlobalVariables.smartCropPoints[EditActionDetails.BOTTOM_RIGHT] =
          new Offset(cropPoint[4].toDouble(), cropPoint[5].toDouble());
          GlobalVariables.smartCropPoints[EditActionDetails.BOTTOM_LEFT] =
          new Offset(cropPoint[6].toDouble(), cropPoint[7].toDouble());
...
//      });
  ...
  ///通过HED模型处理图片，获取二值图片
//  Future<Uint8List> recognizeHed() async {
//    int INPUT_SIZE = 256;
...
//      imageBytes: imageToByteList(image, INPUT_SIZE),
//      outputSize: 4 * INPUT_SIZE * INPUT_SIZE,
//    );
//
//    convertOutputBufferToImage(recognitions, srcWidth, srcHeight);
//    //使用开源库来进行编码，奖rawRgba转为jpg后Widget Image才能显示
//    img.Image im = img.Image.fromBytes(INPUT_SIZE, INPUT_SIZE, recognitions);// format: img.Format.argb);
//    //resize picture
//    im = img.copyResize(im, width:srcWidth, height:srcHeight);
//    return im.getBytes();
//  }

  ///把图片转换为字节list
  ///
  /// image image库中的image类型图片
  /// _inputSize image图片的大小，因为是正方形所以宽高一样
  /// return Uint8List
  Uint8List imageToByteList(img.Image image, int _inputSize) {
    int length = 4 * _inputSize * _inputSize * 3;
    var convertedBytes = new Uint8List( length);
    ...
    for (var i = 0; i < _inputSize; ++i) {
      for (var j = 0; j < _inputSize; ++j) {
        ...
        //AARRGGBB
        buffer.setFloat32(pixelIndex+=4,( (pixel) & 0xFF).toDouble(), Endian.little);
        buffer.setFloat32(pixelIndex+=4, ((pixel >> 8) & 0xFF).toDouble(), Endian.little);
        if(pixelIndex < length - 4){
          buffer.setFloat32(pixelIndex+=4, ((pixel >> 16) & 0xFF).toDouble(), Endian.little);
        }
      }
    }
  ...
  /// srcWidth 图片的宽
  /// srcHeight 图片的高
  void convertOutputBufferToImage(Uint8List outImgData, int srcWidth, int srcHeight) {
    int INPUT_SIZE = 256;
    if (outImgData == null) {
      return null;
    }
    //二值
    int length = INPUT_SIZE * INPUT_SIZE * 4;
    for (int i = 0; i < length; i+=4) {
      //先判断是否为红色，再判断是否为深色
      if(outImgData[i + 3] > 150 || (outImgData[i + 1] < 100 && outImgData[i + 2] < 100 && outImgData[i + 3] < 100)){
        outImgData[i + 1] = 0;
        outImgData[i + 2] = 0;
        outImgData[i + 3] = 0;
      }else{
...
//    );
  ...
  @override
  Uint8List get extendedImageByteData => _imageByteData;
