...
class EditActionDetails {
  ...
  Rect get cropRect {
    ...
    return Rect.fromLTRB(
        min(cropPoints[TOP_LEFT].dx, cropPoints[BOTTOM_LEFT].dx),
        min(cropPoints[TOP_LEFT].dy, cropPoints[TOP_RIGHT].dy),
        max(cropPoints[TOP_RIGHT].dx, cropPoints[BOTTOM_RIGHT].dx),
        max(cropPoints[BOTTOM_LEFT].dy, cropPoints[BOTTOM_RIGHT].dy));
  }
  ...
  void rotate(double angle, Rect layoutRect, BoxFit fit) {
    ...
    Offset cropCenter = cropRect.center;
    Rect newScreenDestinationRect =
        rotateRect(screenDestinationRect, screenCropRect.center, angle);
    double top = cropRect.center.dy - newScreenDestinationRect.height/2;
    double bottom = cropRect.center.dy + newScreenDestinationRect.height/2;

    //从竖直方向旋转下来，调整一下剪裁区域的点
    if(cropRect.height > newScreenDestinationRect.height){
      if(doubleCompare(cropPoints[TOP_LEFT].dy ,top)  < 0){
        cropPoints[TOP_LEFT] = new Offset(cropPoints[TOP_LEFT].dx,cropCenter.dy - newScreenDestinationRect.height/2);
      }
      if(doubleCompare(cropPoints[TOP_RIGHT].dy , top) < 0){
        cropPoints[TOP_RIGHT] = new Offset(cropPoints[TOP_RIGHT].dx,cropCenter.dy  - newScreenDestinationRect.height/2);
      }
      if(doubleCompare(cropPoints[BOTTOM_LEFT].dy , bottom) > 0){
        cropPoints[BOTTOM_LEFT] = new Offset(cropPoints[BOTTOM_LEFT].dx,cropCenter.dy  + newScreenDestinationRect.height/2);
      }
      if(doubleCompare(cropPoints[TOP_RIGHT].dy , bottom) < 0){
        cropPoints[BOTTOM_RIGHT] = new Offset(cropPoints[BOTTOM_RIGHT].dx,cropCenter.dy  + newScreenDestinationRect.height/2);
      }
    }
    debugPrint("cropPoints:" + cropPoints.toString());

    final Offset topLeft = screenCropRect.center -
        (screenCropRect.center - newScreenDestinationRect.topLeft);
